...
- **学习批次**：手动抽取，卡片式学习，支持标记完成 / 撤销完成，顶部进度条；批次不按日期轮换，没背完的一直保留，点"换一批"才刷新（未完成的词保留进新批次）
- **间隔复习（SRS）**：Leitner 盒子法——标记完成（认识）时盒子 +1（上限 5），下次复习间隔按盒子为 1/2/4/8/16 天；到期复习词在"换一批"时最优先进入新批次（可超出每批数量，排在批次前面）
- **自测模式**：学习批次页顶部"浏览模式 / 自测模式"开关；自测模式卡片只显示单词和词性，点击翻转看释义，按"认识"（= 标记完成 + SRS 升盒）或"不认识"（盒子归 0 明天再复习，词保留在本批次并标红提示）
- **单词详情**：
...
- **统计**：总词数、覆盖率、本批进度、累计完成次数、今日到期复习数、SRS 盒子分布（进度条），分类分布与完成次数分布图表
...
4. "设置"页可调整每批抽取数量与冷却天数（对下一次抽取生效）

## 抽取算法说明

点"换一批"（或首次抽取）时生成 N 个词的批次（N=每批数量）：

1. **到期复习词最优先（SRS）**：`next_review_at <= 今天` 且盒子 > 0 的词全部进入新批次（按到期日最旧优先排前，可超出 N）；自测答"不认识"的词盒子归 0、明天到期
2. **未完成的词保留**：上一批未标记完成的词直接进入新批次，学完为止
3. 剩余名额第一优先随机抽**从未抽取过**的词
4. 不足则抽**累计完成次数少**的词（次数升序 → 最久未抽优先）
5. **冷却期**：最近 `冷却天数` 内抽过的词不参与（避免连续重复）
6. 候选不足 N 个时放宽冷却限制补足
